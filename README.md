...
Цели месяца отображаются из распределения финансовых средств.
Тап по дате на мобильном — визуальный отклик (scale + подсветка).
...
- Свайп влево/вправо для навигации между днями/неделями на мобильном
- Адаптивная шкала времени — события видны и читаемы на любом экране

### ✅ Трекер задач
Мини-канбан с четырьмя статусами:
- **Сделать** — задачи к выполнению
- **В работе** — задачи в процессе (с таймером, для удобства можно использовать плагин Tray)
- **На паузе** — приостановленные задачи (таймер на паузе)
- **Готово** — выполненные задачи

Возможности:
- Дедлайны задач
- Проекты с цветовой маркировкой и drag & drop сортировкой в фильтре
- Приоритеты (высокий, средний, низкий)
- Повторяющиеся задачи с датой окончания (ежедневные, еженедельные, ежемесячные)
- Встроенный таймер учёта времени с логами
- **Авто-возобновление таймера** — при перезапуске Obsidian таймер продолжает тикать с того же места
- **Корректный resume** — при переключении "на паузу" → "в работу" таймер показывает суммарное время всех сессий
- Два типа привязки к заметкам:
  - **Обычная задача + привязанная заметка** — ссылка на существующую заметку (при завершении заметка не архивируется)
  - **Задача-заметка** — создание нового .md файла (при завершении заметка архивируется, при удалении — удаляется)
- Кнопка очистки всех повторяющихся задач (из меню "⋯")
- Автоматическая очистка выполненных задач (более 180, чтобы не засорять память)

### 🔁 Трекер привычек
Три типа частоты:
- **Ежедневно** — выполнение каждый день
- **Ежедневно (выбор дней)** — выбор конкретных дней недели (Пн-Вс)
- **Ежемесячно** — выполнение в указанный день месяца (1-31)
...
- **Цели на месяц**: планируем цели (отображаются в календаре для мотивации)
...
[Svelte 3.x](https://svelte.dev/) · [FullCalendar 6.x](https://fullcalendar.io/) · [Luxon 3.x](https://moment.github.io/luxon/) · [TypeScript 4.x](https://www.typescriptlang.org/) · [Jest](https://jestjs.io/) · Canvas API
